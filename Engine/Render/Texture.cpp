...
namespace Blue
{
	...
	{
		// 예외 처리.
		if (!textureData)	// 바인딩할 데이터가 없으면 반환
		{
			return;
		}
	...
	void Texture::LoadTexture(const std::string& name)
	{
		...
		textureData->data = stbi_load(
			path, &textureData->width, &textureData->height, &textureData->channelCount, 0);
		...
		textureDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;	// unsigned normalize
